"""
Gemini AI Service — handles symptom analysis and prescription OCR via Google Gemini.
...
async def analyze_symptoms(
    ...
    """Analyze symptoms using Gemini and return structured medical assessment."""
    client = _get_client()
    if not client:
...
async def extract_prescription(image_bytes: bytes, mime_type: str = "image/jpeg") -> dict:
    """Extract medicines from a prescription image using Gemini Vision."""
    client = _get_client()
    if not client:
        return {"error": "Gemini API key not configured"}
    ...
    try:
        ...
        if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg:
             return {
                ...
                "friendly_message": "Daily scan limit reached. Please try again tomorrow."
             }
        return {"error": f"Gemini Vision error: {error_msg}"}
